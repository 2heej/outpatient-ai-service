...
class IntakeSubmission(BaseModel):
    ...
    disease_context: str = Field(..., examples=DISEASE_CONTEXT_OPTIONS)
    visit_purpose: list[str] = Field(default_factory=list)
    symptom_change: str | None = None
    requested_consultation: str | None = None
    raw_input_text: str | None = None
    document_type: list[str] = Field(default_factory=list)
    document_destination: list[str] = Field(default_factory=list)
